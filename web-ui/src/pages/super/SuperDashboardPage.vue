...
<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-2xl font-bold text-gray-900">{{ t('super.dashboard.title') }}</h1>
      <p class="mt-1 text-sm text-gray-500">{{ t('super.dashboard.subtitle') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-500">{{ t('super.dashboard.totalTenants') }}</p>
      ...
      <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-500">{{ t('super.dashboard.totalUsers') }}</p>
            <p class="mt-1 text-2xl font-bold text-gray-900">
              <span v-if="loading" class="inline-block h-7 w-16 animate-pulse rounded bg-gray-200" />
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
      <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-base font-semibold text-gray-900">{{ t('super.dashboard.tenants') }}</h2>
        </div>
        <router-link
      ...
      <div v-else-if="recentTenants.length === 0" class="px-5 py-12 text-center">
        ...
        <p class="mt-3 text-sm font-medium text-gray-500">{{ t('super.dashboard.noTenants') }}</p>
        <p class="mt-1 text-sm text-gray-400">{{ t('super.dashboard.tenantsWillAppear') }}</p>
        <router-link
          :to="{ name: 'super-tenants' }"
