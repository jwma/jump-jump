...
<template>
  <div>
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-xl font-semibold text-gray-900">{{ t('tenantCreate.title') }}</h1>
        <p class="mt-0.5 text-sm text-gray-500">{{ t('tenantCreate.description') }}</p>
      </div>
    </div>
    ...
    <form class="mt-6 max-w-lg rounded-lg border bg-white p-6" @submit.prevent="handleSubmit">
      <div class="space-y-5">
        ...
        <div>
          ...
          <p v-else class="mt-1 text-xs text-gray-400">{{ t('tenantCreate.slugHint') }}</p>
        </div>
      </div>
