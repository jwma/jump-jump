  ...
  <div class="flex min-h-screen items-center justify-center bg-gray-50">
    <div class="w-full max-w-sm">
      <div class="mb-8 text-center">
        ...
        <h1 class="text-2xl font-bold text-gray-900">{{ t('auth.appName') }}</h1>
        <p class="mt-1 text-sm text-gray-500">{{ t('auth.appTagline') }}</p>
      </div>
      <form class="rounded-lg border bg-white p-6 shadow-sm" @submit.prevent="handleLogin">
        <div class="mb-4">
          <label class="mb-1 block text-sm font-medium text-gray-700">{{ t('auth.username') }}</label>
          <input
            v-model="username"
            type="text"
            required
        ...
        <div class="mb-4 flex items-center">
          ...
          />
          <label for="remember-me" class="ml-2 text-sm text-gray-600">{{ t('auth.rememberMe') }}</label>
        </div>
